...
/**
 * \brief Extreme distribution random variable generator
 * \ingroup randomvariable
 *
 *  Class ExtremeVariable creates a random number generator that
 *  generates random numbers based on the Extreme Distribution
 *
 *  Extreme distribution is defined as follows:
 *      -PDF = 1/b * e^(-(x-a)/b) * e^(-e^(-(x-a)/b))
 *      -CDF = e^(-e^(-(x-a)/b))
 *      (a = location, b = scale)
 *
 */

class ExtremeVariable : public RandomVariable
{
public:
     ...
     */
    ExtremeVariable (double loc, double scale);

    /**
     * \brief Contructs an Extreme value random variable with upper bound
     *  \param loc Location parameter
     *  \param scale Scale parameter
     *  \param bound Upper bound of returned values
     */
    ExtremeVariable (double loc, double scale, double bound);
...
public:
  ...
  * \param loc Location parameter
  */
  WeibullVariable (double m, double s, double b, double loc);
...
/**
 * \brief Split distribution, that is constructed from multiple individual probability distributions
 * \ingroup randomvariable
 *
 * A probability distribution, that is constructed from multiple individual distributions.
 * Each of the distributions have a percentage value p (0<=p<=1), that multiplies the PDF
 * in order to balance the distributions with each other. This also ensures that the
 * probability density of all distribution is equal to 1, as normally each of the probability
 * distributions would have probability density of 1.
 */
class SplitDistribution : public RandomVariable
{
public:
    SplitDistribution ();
