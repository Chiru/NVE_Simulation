...
StatisticsCollector::StatisticsCollector(bool verbose, bool clientLog, bool serverLog, uint16_t numberOfStreams, int runningTime):
    streamCount(numberOfStreams),
    runningTime(runningTime),
    scriptSourceFile("results/rscriptfile.R"),
    scriptResultPdfFile("results/resultgraphs.pdf"),
    scriptResultTextFile("results/results.txt")
{
...
StatisticsCollector::~StatisticsCollector()
{
    ...
    {
        getStreamResults(userActionMessageLog[h], serverMessageLog[h], h+1, singleStreamClientToClient, singleStreamClientToServer,singleStreamServerToClient,
                         clientToClientMsgCount, clientToServerMsgCount,serverToClientMsgCount, toServerInTime, fromClientToClientInTime, fromServerToClientInTime,
                         clientToServerTimes[h], clientToClientTimes[h], serverToClientTimes[h]);
    ...
    PRINT_RESULT("Overall stats: "  <<"\n\tAverage transmission times   clientToServer: " <<  averageClientToServer.GetMilliSeconds() << "   clientToClient: "
                 ...
                 << averageClientToClientInTime << "\n\tAverage percentage of messages from server to reach client in time: "
                 << averageServerToClientInTime << std::endl);
...
void StatisticsCollector::logMessagesSentFromClient(int messageNumber, Time sendTime, uint16_t streamNumber, uint32_t clientTimeRequirement,
                                                    uint32_t serverTimeRequirement, uint16_t nameIndex, uint16_t messageId, uint16_t size)
{
...
void StatisticsCollector::logMessagesSentFromServer(int messageNumber, Time sendTime, uint16_t streamNumber, uint32_t clientTimeRequirement,
                                                    uint16_t messageNameIndex, uint16_t messageId, uint16_t size)
{
    static std::map<int, Time> lastTimes;
...
void StatisticsCollector::getStreamResults(std::vector<StatisticsCollector::MessageStats*>& clientStats, std::vector<StatisticsCollector::MessageStats*>& serverStats,
                                           ...
                                           uint32_t& clientToClientMsgCount, uint32_t& clientToServerMsgCount, uint32_t& serverToClientMsgCount,  uint32_t& toServerInTime,
                                           uint32_t& fromClientToClientInTime, uint32_t& fromServerToClientInTime,std::list<int64_t>& clientToServerTimes,
                                           std::list<int64_t>& clientToClientTimes, std::list<int64_t>& serverToClientTimes)
{
...
void StatisticsCollector::getBandwidthResults()
{
    ...
    for(addrIt = nodesAndBandwidths.begin(), counter = 0; addrIt != nodesAndBandwidths.end(); addrIt++)
    {
        ...
        {
            runningTime = clientsInformation[addrIt->first].runningTime;
...
void StatisticsCollector::getMessageStats(uint16_t streamNumber)
{
    ...
    uint32_t* messagesSentFromServer =(uint32_t*)calloc(otherDataMessageCount, sizeof(uint32_t));
        ...
        for(int h = 0; h < userActionmessageCount; h++)
        {
            ...
            {
                scriptGen->generateScriptForClientMessage(messageRecvTimesForClientToClient[h].second, messageRecvTimesForServer[h].second,
                                                          messageSendIntervalsFromClient[h], messageRecvTimesForServer[h].first.first,
                                                          messageRecvTimesForServer[h].first.second, messageRecvTimesForClientToClient[h].first.second,
        ...
        for(int h = 0; h < otherDataMessageCount; h++)
        {
            ...
            {
                scriptGen->generateScriptForServerMessage(messageRecvTimesFromServerToClient[h].second, messagesSendIntervalFromServer[h],
                                                          messageRecvTimesFromServerToClient[h].first.first, messageRecvTimesFromServerToClient[h].first.second,
                                                          messagesSentFromServer[h], odmSizes[h].second);
            }
        }
...
void StatisticsCollector::setServerPcap(bool pcap)
{
    serverPcap = pcap;
}
...
bool StatisticsCollector::generateSingleNodeStatsFromPcap()
{
    ...
    for(std::map<Ipv4Address, ClientInfo>::const_iterator it = clientsInformation.begin(); cont; it++)
    {

        if(it == clientsInformation.end())
        {
            cont = false;
            isServer = true;
            addr = serverAddr;
...
bool StatisticsCollector::generateOverallGraphFromPcap()
{
        ...
        if(pcap)
        {
            ...
            if((pcapFile.st_mode & S_IFMT) == S_IFREG)   //if pcap-file exists
            {
                if((pid = fork()) == 0)
                {
                    ...
                    const char* const args[] = {"tshark", "-r", fileName.c_str(), "-qz", statsArg.c_str(), NULL};
                    execvp("tshark", (char* const*)args);
                }
    ...
    }while(it != clientsInformation.end());
