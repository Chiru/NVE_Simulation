...
ApplicationProtocol::ApplicationProtocol(uint16_t packetSize, uint16_t delayedAck, uint16_t retransmit, uint16_t headerSize)
    : ackSize(packetSize),
      ...
      socket(0),
      maxDatagramSize(1400) //hard coding
{
}
...
bool ApplicationProtocol::sendFromClient(const Message *msg, uint8_t *buffer, Ptr<Socket> socket, bool forward)
{
    ...
    if(socket->GetTxAvailable() < messageSize + headerSize)
    {
        return false;
    }
...
bool ApplicationProtocol::sendFromClient(const std::string &buffer, Ptr<Socket> socket, bool reliable)
{
    ...
    if(socket->GetTxAvailable() < buffer.length() + headerSize)
    {
        delete [] msgContents;
        return false;
    }
...
bool ApplicationProtocol::sendFromServer(uint8_t *buffer, const Message* msg, const Address& addr, Ptr<Socket> socket, bool forward)
{
    ...
    char msgContents[size + headerSize];
    ...
    if(socket->GetTxAvailable() < size + headerSize)
    {
        return false;
    }
...
bool ApplicationProtocol::sendFromServer(const std::string &buffer, const Address &addr, Ptr<Socket> sock, bool reliable)
{
    ...
    if(sock->GetTxAvailable() < size + headerSize)
    {
        delete [] msgContents;
        return false;
    }
...
void ApplicationProtocol::ackAllPackets()
{
    ...
    for(std::map<Address, std::deque<uint32_t> >::iterator it = packetsToAck.begin(); it != packetsToAck.end(); it++)
    {
        ...
        if(sendAck(messages, i, it->first, socket))
        {
...
int ApplicationProtocol::sendFragment(const std::string& buffer, const size_t index, Ptr<Socket> sock, uint16_t maxDatagramSize, const Address* const addr,
                                      ApplicationProtocol* appProto, bool reliable, uint16_t headerSize)
{
    size_t tempIndex = 0, tempIndex2 = 0;
    tempIndex = buffer.find('"', index);   //find first "
    tempIndex = buffer.find('"', tempIndex + 1);  // find second "

    if((tempIndex = buffer.find('"', tempIndex + 1)) == std::string::npos)  //see if there's no third
    {
        if(addr == 0)
        {
            if(reliable)
                appProto->rememberReliablePacket(appProto->reliableMsgNumber.begin()->second, buffer.length(),
                                                 (uint8_t*)buffer.substr(0, buffer.length()).c_str(), &ApplicationProtocol::resendCheckClient);
        ...
        {
            if(reliable)
               appProto->rememberReliablePacket(appProto->reliableMsgNumber, buffer.length(), (uint8_t*)buffer.substr(0, buffer.length()).c_str(),
                                                *addr, &ApplicationProtocol::resendCheckServer);
    ...
    else
    {
        if( tempIndex >= maxDatagramSize || (tempIndex2 = buffer.find('"', tempIndex + 1)) >= maxDatagramSize || buffer.length() > maxDatagramSize)
        {
            ...
            {
                if(reliable)
                    appProto->rememberReliablePacket(appProto->reliableMsgNumber.begin()->second, tempIndex, (uint8_t*)sendString.c_str(),
                                                     &ApplicationProtocol::resendCheckClient);
            ...
            {
                if(reliable)
                    appProto->rememberReliablePacket(appProto->reliableMsgNumber, tempIndex, (uint8_t*)sendString.c_str(), *addr,
                                                     &ApplicationProtocol::resendCheckServer);
...
void ApplicationProtocol::rememberReliablePacket(uint32_t messageNumber, uint16_t messageSize, uint8_t *messageContents,
                                                 void (ApplicationProtocol::*fptr)(uint32_t))
{
    uint8_t* tempMsg;
...
void ApplicationProtocol::rememberReliablePacket(std::map<const Address, uint32_t> & reliableMsgNumber, uint16_t messageSize, const uint8_t *messageContents,
                                                 const Address &addr, void (ApplicationProtocol::*fptr)(std::map<const Address, uint32_t>&, const Address&))
{
    uint8_t* tempMsg;
    tempMsg = (uint8_t*)malloc(messageSize + headerSize);
    memcpy(tempMsg, messageContents, messageSize + headerSize);
